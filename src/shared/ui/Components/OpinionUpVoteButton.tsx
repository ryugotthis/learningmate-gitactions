  ...
  return (
    <>
      <button
        ...
        className={`flex w-[64px] h-[92px] flex-col justify-center gap-1 items-center border-2 rounded-4xl cursor-pointer ${
          isLikeClicked ? 'border-primary-default' : 'border-surface-line'
        }`}
      >
        <UpIcon className="text-surface-line w-[24px]" />
        <span className="font-semibold tracking-[-0.05em]">{likes}</span>
      </button>
    </>
  );
};
