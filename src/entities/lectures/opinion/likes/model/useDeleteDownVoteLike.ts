...
import { deleteDownVoteLike } from '../api/deleteDownVoteLike';

export const useDeleteDownVoteLike = (postId: number) => {
  // const navigate = useNavigate();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (opinionId: number) => deleteDownVoteLike(opinionId),
    ...
    onMutate: async (opinionId: number) => {
      ...
      queryClient.setQueryData(['downVoteLikeStatus', opinionId], false);
