...
import { createUpVoteLike } from '../api/createUpVoteLike';

export const useCreateUpVoteLike = (postId: number) => {
  // const navigate = useNavigate();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (opinionId: number) => createUpVoteLike(opinionId),
    ...
    onMutate: async (opinionId: number) => {
      ...
      queryClient.setQueryData(['upVoteLikeStatus', opinionId], true);
