...
export const customGet = async (
  ...
  try {
    const response = await apiClientCustom.get(`/${url}`); // Interceptor 적용된 apiClient 사용
    return response.data;
  } catch (error) {
    console.log('api get 실패1:', error);
    throw error;
  }
};
...
export const customPost = async (
  ...
  try {
    const response = await apiClientCustom.post(`/${url}`, data);
...
//   const apiClient = useApiClient(); // useApiClient 호출

//   return async (url: string): Promise<any> => {
//     try {
//       const response = await apiClient.get(`/${url}`); // Interceptor 적용된 apiClient 사용
//       return response.data;
//     } catch (error) {
//       console.log('api get 실패1:', error);
//       throw error;
